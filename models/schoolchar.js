...
const SchoolSchema = mongoose.Schema ({
  ...
  start_date: {
    type: Date,
    default: new Date (2018, 8, 1),
  },
  end_date: {
    type: Date,
    default: new Date (2019, 5, 30),
  },
  year_day_object: {
    type: Object,
  },
...
SchoolSchema.pre ('save', async next => {
  try {
    ...
    for (var i = 0; i < blockLetters.length; i++) {
      let block = await models.block.create ({
        school: school._id,
        block: blockLetters[i][0],
        is_constant: blockLetters[i][1] === 'constant',
      });
    ...
    school.schedule = [
      {
        block_times: [
          {start_hour: 9, start_minute: 10, end_hour: 10, end_minute: 12},
        ],
        day_blocks: [
          {
